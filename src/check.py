...
check_path_list = ["lang", "config", "config/basic.ini"]
make_list = ["data?dir"]
...
def check():
    ...
    if result == 1:
        sg.PopupError(f"Error : Not found pass({error_path})")
        return 2
    elif result == 2:
        sg.PopupError(f"Error : Fail make pass({error_path})")
        return 3
    # Network check
    result = network()
    if result != 0:
        sg.PopupError("Error : Not connected to network")
        return 4
    # Check ini file
    result = ini()
    if result != 0:
        sg.PopupError("Error : Fail load ini file")
        return 5
    # Check lang file
    result = lang()
    if result != 0:
        sg.PopupError("Error : Fail load lang file")
        return 6
    return 0
